...
NB: {{< fa brands apple >}} **NVivo for macOS**: Nvivo specifies that you use a csv file rather than an excel file, which you will find in the folder too. However the same situation applies - you need to ensure that the cases you have created are in a similar format to the csv file. That is, if they are all in lower case, the csv file must also be all in lower case. If the cases are capitalised (e.g. 'Vince' the csv file must also 'Vince'). 
...
- Select the ‘metadata.xls’ file from the ‘Research tools’ folder. ({{< fa brands apple >}} **NVivo for macOS**: import the csv file instead.)
...
{{< fa brands apple >}} **NVivo for macOS**: this import wizard looks a bit different.

![](images/case class mac.png)

- Set classification type as ‘case classification’.

 {{< fa brands apple >}} **NVivo for macOS**: here create new case classification 'People' (Windows users have done this step earlier).
...
NB. {{< fa brands apple >}} **NVivo for macOS**: This option is not available on macOS - just click through. 
...
NB. {{< fa brands apple >}} **NVivo for macOS**: Step 4 is not available on mac, just click through. 
...
{{< fa brands apple >}} **NVivo for macOS**: the stages of setting up a new case are similar, but the menus might look a bit different. 
...
### Summarise metadata in charts (Windows only)

::: macos

These instructions below are for Windows users. Mac users do have chart functions, but they are more limited.
