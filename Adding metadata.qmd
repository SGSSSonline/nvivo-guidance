...
Now that we have our cases, we can create  ‘case classifications’, bringing in the metadata related to each file or individual. 
...
- The names in this first column should match the formatting of the cases and the interview file names in Nvivo - and this is **case sensitive** (i.e. if it's 'charles' it must also be 'charles' and not 'Charles') 
...
![](Images/class wizard step 4.png)

Click finish.

Check that case info has been imported to existing ‘People’ classification. You can see that the first row of the excel sheet linked with the cases that already existed, and now you have columns indicating exactly the columns that were in the spreadsheet. 

![](Images/check case classification.png)
